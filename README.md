...
All Kontent.ai test enviroments are exported in `tests/integration/<testName>/data/<zipName>.zip`. When you update any of these environments, you should also update the corresponding exported zip files. To streamline this process, we've provided a script called `exportTestEnvironments.js`.  To use it you can use `npm run export:testEnv`. You can run it with the command `npm run export:testEnv`. If you need to export only specific environments, you can use command parameters. For example, use `npm run export:testEnv -- -s -t` to export only the Sync Source and Sync Target environments.
