...
export const createTransformTypeElement = (params: TransformTypeElementParams) => (element: ElementContracts.IContentTypeElementContract): ContentTypeElements.IElementShared => {
  ...
  const content_group = elementWithGroup.content_group ? { external_id: params.contentGroupExternalIdByOldId.get(elementWithGroup.content_group.id ?? "") } : undefined;
  ...
  switch (element.type) {
    case "asset": {
      ...
      return params.builder.assetElement({
        ...
        content_group,
        default: typedElement.default ? { global: { value: typedElement.default.global.value.map(ref => ({ id: params.context.assetIdsByOldIds.get(ref.id ?? "") })) } } : undefined,
      });
    }
    case "custom": {
      ...
      return params.builder.customElement({
        ...
        external_id: typedElement.external_id ?? fallbackExternalId,
        content_group,
        allowed_elements: typedElement.allowed_elements?.map(ref => ({ external_id: params.elementExternalIdsByOldId.get(ref.id ?? "") })),
      });
    }
    case "date_time": {
    ...
    case "multiple_choice": {
      ...
      return params.builder.multipleChoiceElement({
        ...
        default: typedElement.default ? { global: { value: typedElement.default.global.value.map(ref => {
          const oldOption = typedElement.options.find(o => o.id === ref.id);
          if (!oldOption) {
            throw new Error(`Default value of a multiple_choice element (codename: "${typedElement.codename}")contains a non-existent option.`);
          }
    ...
    case "snippet": {
      if (!("snippetElement" in params.builder)) {
        throw new Error(`Type snippet with codename "${params.typeOrSnippetCodename}" has an element (codename: "${element.codename}") of type snippet which is not allowed.`);
      }
      ...
      return params.builder.snippetElement({
        ...
        external_id: typedElement.external_id ?? fallbackExternalId,
        content_group,
        snippet: { id: params.context.contentTypeSnippetIdsWithElementsByOldIds.get(typedElement.snippet.id ?? "")?.selfId },
      });
    }
    case "subpages": {
    ...
    case "taxonomy": {
      ...
      return params.builder.taxonomyElement({
        ...
        default: typedElement.default ? { global: { value: typedElement.default.global.value.map(ref => ({ id: params.context.taxonomyTermIdsByOldIds.get(ref.id ?? "") })) } } : undefined,
      });
    }
    case "text": {
    ...
    case "url_slug": {
      ...
      if (!("urlSlugElement" in params.builder)) {
        throw new Error(`Type snippet with codename "${params.typeOrSnippetCodename}" has en element (codename: "${element.codename}") of type url_slug which is not allowed.`);
      }
...
export const createPatchItemAndTypeReferencesInTypeElement = (context: ImportContext, pickElementsByOldIds: (context: ImportContext) => ReadonlyMap<string, string> | undefined) =>
  (fileElement: ElementContracts.IContentTypeElementContract): ReadonlyArray<ContentTypeSnippetModels.IModifyContentTypeSnippetData> => {
    const newElementId = pickElementsByOldIds(context)?.get(fileElement.id ?? "");
    if (!newElementId) {
      throw new Error(`Failed to find new element id for element (codename: "${fileElement.codename}", id: "${fileElement.id}").`);
    }
