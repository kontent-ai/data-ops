import type { CollectionContracts, CollectionModels } from "@kontent-ai/management-sdk";
...
export const collectionsEntity = {
  ...
  importEntities: async (client, { entities: fileCollections, context }) => {
    ...
    if (matchErrors.length) {
      throw new Error(
        `Cannot import collections due to errors: ${matchErrors.map((e) => `"${e.error}"`).join(", ")}.`,
      );
    }
    ...
    return {
      ...
      collectionIdsByOldIds: new Map([
        ...collectionsToUpdate.map(
          (match) => [match.fileCollection.id, match.projectCollection.id] as const,
        ),
        ...newCollections
          .map((c) => {
...
const findCollectionMatches = (
  ...
  fileCollections.flatMap((fileCollection) =>
    projectCollections.map((projectCollection) =>
      matchCollections(fileCollection, projectCollection),
    ),
  );
...
const isMatchError = (matchResult: MatchResult): matchResult is Readonly<{ error: string }> =>
  "error" in matchResult;

const isMatch = (
  matchResult: MatchResult,
): matchResult is Readonly<{
  match: true;
...
const matchCollections = (
  ...
  const externalIdComparison = compareExternalIds(
    projectCollection.external_id,
    fileCollection.external_id,
  );

  if (!hasSameCodename && externalIdComparison === "Same") {
    return {
      error: `Cannot update codename of collections. Collections with external id "${fileCollection.external_id}" have different codenames (file: "${fileCollection.codename}", project: "${projectCollection.codename}").`,
    };
  }
  if (!hasSameCodename) {
    return { match: false };
  ...
  switch (externalIdComparison) {
    ...
    case "Different":
      return {
        error: `Cannot update external id of collections. Collections with codename "${fileCollection.codename}" have different external ids (file: "${fileCollection.external_id}", project: "${projectCollection.external_id}")`,
      };

    case "OnlyProjectUndefined":
      return {
        error: `Cannot update external id of collections. Collection with external id "${fileCollection.external_id}" has the same codename ("${fileCollection.codename}") as a collection in the project that has no external id.`,
      };
