...
import type { MapValues, Replace, ReplaceReferences, RequiredId } from "../../../../utils/types.js";
...
export const contentTypesEntity = {
  ...
  importEntities: async (client, { entities: fileTypes, context, logOptions }) => {
    const projectTypes = await serially(
      fileTypes.map(createInsertTypeFetcher({ context, client, logOptions })),
    );

    return {
      ...context,
      contentTypeContextByOldIds: new Map(
        zip(fileTypes, projectTypes).map(createMakeTypeContextByOldIdEntry(context)),
      ),
    };
  },
...
export const updateItemAndTypeReferencesInTypesImportEntity = {
  ...
  importEntities: async (client, { entities: fileTypes, context, logOptions }) => {
    await serially(
      fileTypes.map(createUpdateTypeItemReferencesFetcher({ client, context, logOptions })),
    );
...
const createMakeTypeContextByOldIdEntry =
  (context: RestoreContext) =>
  ([fileType, projectType]: readonly [Type, Type]): readonly [
    string,
    MapValues<RestoreContext["contentTypeContextByOldIds"]>,
  ] => {
    const elementIdEntries = zip(fileType.elements, projectType.elements).flatMap(
      ([fileEl, projectEl]) => {
        ...
        elementTypeByOldIds: new Map(
          fileType.elements.flatMap((el) => {
            if (el.type === "snippet") {
              ...
              return [
                ...getRequired(
                  context.contentTypeSnippetContextByOldIds,
                  typedEl.snippet.id,
                  "snippet",
                ).elementTypeByOldIds,
              ];
          ...
          fileType.elements.flatMap((el) => {
            switch (el.type) {
              ...
              case "multiple_choice": {
                ...
                const typedProjectEl = projectType.elements.find(
                  (e) => e.codename === el.codename,
                ) as MultiChoiceElement;

                return [
                  [
                    el.id,
                    new Map(
                      zip(typedEl.options, typedProjectEl.options).map(([fO, pO]) => [
                        fO.id,
                        pO.id,
                      ]),
                    ),
...
const createInsertTypeFetcher = (params: InsertTypeParams) => (type: Type) => () => {
  ...
  const makeGroupFallbackExternalId = (groupCodename: string | undefined) =>
    `${type.codename}_${groupCodename}_group`;

  const getElementContentGroupCodename = (element: TypeElement) =>
    type.content_groups?.find(
      ...
      elements: type.elements.map(
        createTransformTypeElement({
          ...
          contentGroupExternalIdByOldId: new Map(
            type.content_groups?.map(
              (g) => [g.id, g.external_id ?? makeGroupFallbackExternalId(g.codename)] as const,
            ) ?? [],
          ),
        }),
      ),
