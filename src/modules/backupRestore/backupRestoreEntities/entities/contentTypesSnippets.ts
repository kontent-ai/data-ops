  ...
  importEntities: async (client, { entities: fileSnippets, context, logOptions }) => {
    ...
    return {
      ...
      elementTypesByOldElementIdsByOldSnippetIds: new Map(
        fileSnippets.map((snippet) => [
          snippet.id,
          new Map(snippet.elements.map((el) => [el.id, el.type])),
        ]),
      ),
    };
  },
...
export const updateItemAndTypeReferencesInSnippetsImportEntity = {
  ...
  importEntities: async (client, { entities: fileSnippets, context, logOptions }) => {
    await serially(
      fileSnippets.map(
        createUpdateSnippetItemAndTypeReferencesFetcher({ client, context, logOptions }),
      ),
    );
...
const makeSnippetContextByOldIdEntry = ([fileSnippet, projectSnippet]: readonly [
  Snippet,
  Snippet,
]): readonly [string, MapValues<RestoreContext["contentTypeSnippetContextByOldIds"]>] => {
  const elementIdEntries = zip(fileSnippet.elements, projectSnippet.elements).map(
    ([fileEl, projectEl]) => [fileEl.id, projectEl.id] as const,
  );
  ...
  return [
    fileSnippet.id,
    {
      ...
      multiChoiceOptionIdsByOldIdsByOldElementId: new Map(
        fileSnippet.elements.flatMap((el) => {
          if (el.type !== "multiple_choice") {
            return [];
          }

          const typedEl = el as MultiChoiceElement;
          const projectTypedEl = projectSnippet.elements.find(
            (e) => e.codename === el.codename,
          ) as MultiChoiceElement;
          const multiChoiceOptionEntries = zip(typedEl.options, projectTypedEl.options).map(
            ([fO, pO]) => [fO.id, pO.id] as const,
          );
...
const createUpdateSnippetItemAndTypeReferencesFetcher =
  (params: UpdateSnippetParams) => (snippet: Snippet) => () => {
    const patchOps = snippet.elements.flatMap(
      createPatchItemAndTypeReferencesInTypeElement(
        params.context,
        getRequired(params.context.contentTypeSnippetContextByOldIds, snippet.id, "snippet")
          .elementIdsByOldIds,
        params.logOptions,
      ),
    );
