...
type SyncContentFilterDeliveryOnlyParams =
  | SuperiorOmit<Exclude<MigrateContentFilterParams, { items: ReadonlyArray<string> }>, "sourceDeliveryPreviewKey">
  | Readonly<
    { items: ReadonlyArray<string>; depth: number; limit?: number; language: string }
  >;
...
export const getDeliveryUrlParams = (
  ...
  return match(params)
    ...
    .with(
      { filter: P.nonNullable },
      ({ filter }) => createDeliveryUrlParameters({ ...defaultParams, customFilter: filter }),
    )
