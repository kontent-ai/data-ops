...
export const makeObjectHandler =
  ...
  (sourceValue, targetValue) => {
    return (
      ...
      return handler(sourceValue[key], targetValue[key]).map(prefixOperationPath(key));
    });
  };
...
export const makeBaseArrayHandler =
  ...
  (sourceValue, targetValue) => {
    ...
    const addAndUpdateOps = sourceValue.flatMap((v) => {
      ...
      if (targetEntity) {
        return [...getCreateUpdateOps()(v, targetEntity).map(prefixOperationPath(getCodename(v)))];
      }
...
export const makeProvideHandler =
  <Entity>(makeHandler: (source: Entity, target: Entity) => Handler<Entity>): Handler<Entity> =>
  (source, target) =>
...
export const makeUnionHandler =
  <DiscriminatorKey extends string, Union extends { [key in DiscriminatorKey]: string }>(
