...
const createGetReferences = (regex: RegExp) => (guidelines: string): ReadonlyArray<OriginalReference> =>
  [...guidelines.matchAll(regex)]
    .map(
      match => ({
...
const createReplaceReferences =
  (params: ReplaceReferencesParams) => (guidelines: string, replacer: (reference: OriginalReference) => Replacement) =>
    guidelines
      .replaceAll(params.referencesRegex, (match, ...[, , , , , , groups]) => { // In the arguments we must first skip the groups (4), offset and the whole string, then there is the groups object. See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace#specifying_a_function_as_the_replacement for more details
        ...
        const newValue = typeof result === "string"
          ? result
        ...
        return match
          .replaceAll(new RegExp(`${params.codenameAttributeName}="[^"]*"`, "gi"), externalId ? "" : newValue) // only put here the new value when there is not external id attribute, otherwise the new value will be put in place of the external id
...
const itemReferenceRegex = new RegExp(
  `<${anythingButItemAttributes}${
    oneOfItemAttributes(codenameGroupName, externalIdGroupName)
...
const assetReferenceRegex = new RegExp(
  `<${anythingButAssetAttributes}${
    oneOfAssetAttributes(codenameGroupName, externalIdGroupName)
