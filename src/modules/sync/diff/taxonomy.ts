...
export const wholeTaxonomyGroupsHandler: Handler<ReadonlyArray<TaxonomySyncModel>> =
  makeWholeObjectsHandler();
              ...
              ? [
                  {
                    ...
                    path: "",
                    before: {
                      codename: topLevelTermCodename ?? throwError(noTopLevelWhenMovigToTopErrMsg),
                    },
                  },
                ]
              : [
  ...
  (ops: ReadonlyArray<PatchOperation>): ReadonlyArray<PatchOperation> => {
    ...
    return ops.map((op) => {
      ...
      return removePositionProp(
        foundTarget ? { ...op, path: termPositionToPath(foundTarget.position) + op.path } : op,
      );
    });
  };
