...
export const transformTypeToAddModel =
  ...
  (type: ContentTypeSyncModel): RequiredCodename<ContentTypeModels.IAddContentTypeData> => ({
    ...
    elements: type.elements.map(transformElementToAddModel(params)) as ContentTypeElements.Element[],
  });

export const transformSnippetToAddModel =
  (params: TransformElementParams) =>
  (snippet: ContentTypeSnippetsSyncModel): RequiredCodename<ContentTypeSnippetModels.IAddContentTypeSnippetData> => ({
    ...snippet,
    elements: snippet.elements.map(transformElementToAddModel(params)) as ContentTypeElements.Element[],
  });

type TransformElementParams = Readonly<{
  targetItemsReferencedFromSourceByCodenames: ReadonlyMap<string, Readonly<{ id: string; codename: string }>>;
  targetAssetsReferencedFromSourceByCodenames: ReadonlyMap<string, Readonly<{ id: string; codename: string }>>;
}>;

const transformElementToAddModel = (params: TransformElementParams) =>
(
  el: ContentTypeSyncModel["elements"][number],
): ContentTypeElements.Element | { type: "subpages"; default?: ContentTypeElements.ILinkedItemsElement["default"] } => { // to fix SDK types bug
  switch (el.type) {
    case "number":
    case "snippet":
    case "url_slug":
...
export const transformGuidelinesElementToAddModel = (
  ...
  const guidelinesWithTransformedAssetReferences = replaceAssetReferences(element.guidelines, ref => {
    const targetAsset = params.targetAssetsReferencedFromSourceByCodenames.get(ref.codename ?? "");

    return targetAsset ? { internalId: targetAsset.id } : { externalId: ref.externalId ?? "" };
  });
  return ({
    ...element,
    guidelines: replaceItemReferences(
...
type MakeArraysMutable<T extends object> = {
  [Key in keyof T]: T[Key] extends ReadonlyArray<unknown> | undefined ? MakeArrayMutable<T[Key]>
    : T[Key];
};

type MakeArrayMutable<T extends ReadonlyArray<unknown> | undefined> = T extends ReadonlyArray<infer Elem> ? Elem[]
  : T;
