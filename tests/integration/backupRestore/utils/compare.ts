...
export type FilterParam =
  | { include: ReadonlyArray<keyof AllEnvData> }
  | { exclude: ReadonlyArray<keyof AllEnvData> };
...
export const expectSameAllEnvData = (
  ...
  const sortedVariants = (data: AllEnvData) =>
    sortBy(data.variants, (v) => `${v.item.id};${v.language.id}`);

  // disabling unused expressions here as they are a lot more compact then ifs and a lot of them is necessary here
  /* eslint-disable @typescript-eslint/no-unused-expressions */
  has("collections") &&
    expect(sortByCodename(data1.collections)).toStrictEqual(sortByCodename(data2.collections));
  has("spaces") && expect(sortByCodename(data1.spaces)).toStrictEqual(sortByCodename(data2.spaces));
  has("languages") &&
    expect(sortByCodename(data1.languages)).toStrictEqual(sortByCodename(data2.languages));
  has("previewUrls") && expect(data1.previewUrls).toStrictEqual(data2.previewUrls);
  has("taxonomies") &&
    expect(sortByCodename(data1.taxonomies)).toStrictEqual(sortByCodename(data2.taxonomies));
  ...
  has("roles") &&
    expect(sortBy(data1.roles, (r) => r.name)).toStrictEqual(sortBy(data2.roles, (r) => r.name));
  has("workflows") &&
    expect(sortByCodename(data1.workflows)).toStrictEqual(sortByCodename(data2.workflows));
  has("snippets") &&
    expect(sortByCodename(data1.snippets)).toStrictEqual(sortByCodename(data2.snippets));
  has("types") &&
    expect(
      sortByCodename(
        data1.types.map((t) => ({
...
export const expectDifferentAllEnvData = (
  ...
  has("spaces") &&
    expect(sortByCodename(data1.spaces)).not.toStrictEqual(sortByCodename(data2.spaces));
  has("languages") &&
    expect(sortByCodename(data1.languages)).not.toStrictEqual(sortByCodename(data2.languages));
  has("previewUrls") && expect(data1.previewUrls).not.to.toStrictEqual(data2.previewUrls);
  has("taxonomies") &&
    expect(sortByCodename(data1.taxonomies)).not.toStrictEqual(sortByCodename(data2.taxonomies));
  has("assetFolders") && expect(data1.assetFolders).not.toStrictEqual(data2.assetFolders);
  has("assets") &&
    expect(sortByCodename(data1.assets)).not.toStrictEqual(sortByCodename(data2.assets));
  has("roles") &&
    expect(sortBy(data1.roles, (r) => r.name)).not.toStrictEqual(
      sortBy(data2.roles, (r) => r.name),
    );
  has("workflows") &&
    expect(sortByCodename(data1.workflows)).not.toStrictEqual(sortByCodename(data2.workflows));
  has("snippets") &&
    expect(sortByCodename(data1.snippets)).not.toStrictEqual(sortByCodename(data2.snippets));
  has("types") &&
    expect(
      sortByCodename(
        data1.types.map((t) => ({
  ...
  has("items") &&
    expect(sortByCodename(data1.items)).not.toStrictEqual(sortByCodename(data2.items));
...
const sortByCodename = <T extends { readonly codename: string }>(
  ...
  languages: data.languages
    ...
    .map((l, _, allLanguages) => ({
      ...
      fallback_language: l.fallback_language
        ? {
            id:
              allLanguages.find((e) => e.id === l.fallback_language?.id)?.codename ??
...
> = (data) => (previewUrls) => ({
  ...
  preview_url_patterns: previewUrls.preview_url_patterns.map((pattern) => ({
    ...pattern,
    content_type: {
      id: data.types.find((t) => t.id === pattern.content_type.id)?.codename ?? "non-existing-type",
    },
    url_patterns: pattern.url_patterns.map((p) => ({
      ...p,
      space: {
        id: data.spaces.find((s) => s.id === p.space?.id)?.codename ?? "non-existing-space",
      },
    })),
  })),
});

const prepareTaxonomyReferences: PrepareReferencesFnc<TaxonomyContracts.ITaxonomyContract> = (
  taxonomy,
) => ({
  ...taxonomy,
  id: "-",
...
const createPrepareAssetReferences: PrepareReferencesCreator<AssetContracts.IAssetModelContract> =
  (data) => (asset) => ({
    ...
    folder: asset.folder
      ? { id: data.assetFolders.find((f) => f.id === asset.folder?.id)?.name }
      : undefined,
    collection: asset.collection
      ? {
          reference: {
            id:
              data.collections.find((c) => c.id === asset.collection?.reference?.id)?.codename ??
    ...
    descriptions: asset.descriptions.map((d) => ({
      ...
      language: {
        id: data.languages.find((l) => l.id === d.language.id)?.codename ?? "non-existing-language",
      },
    })),
  });

const createPrepareWorkflowReferences: PrepareReferencesCreator<
...
> = (data) => (workflow) => ({
  ...
  scopes: workflow.scopes.map((s) => ({
    ...s,
    collections: s.collections.map((ref) => ({
      id: data.collections.find((c) => c.id === ref.id)?.codename ?? "non-existing-collection",
    })),
    content_types: s.content_types.map((ref) => ({
      id: data.types.find((t) => t.id === ref.id)?.codename ?? "non-existing-type",
    })),
  })),
  archived_step: {
  ...
  steps: workflow.steps.map((s) => ({
    ...
    transitions_to: s.transitions_to.map((t) => ({
      step: {
        id: workflow.steps.find((step) => step.id === t.step.id)?.codename ?? "non-existing-step",
      },
    })),
  })),
});
...
> = (data) => (snippet) => ({
  ...
  last_modified: "-",
  external_id: "-",
  elements: snippet.elements.map(createPrepareTypeElementReferences(data, snippet.elements, [])),
});

const createPrepareTypeReferences: PrepareReferencesCreator<
...
> = (data) => (type) => ({
  ...
  elements: type.elements.map(
    createPrepareTypeElementReferences(data, type.elements, type.content_groups ?? []),
  ),
});
...
const createPrepareTypeElementReferences =
  ...
  (element) => {
    const elementWithGroup = element as ElementContracts.IContentTypeElementContract &
    ...
    const baseElement: ElementContracts.IContentTypeElementContract & ContentTypeElements.Element =
      {
        ...
        content_group: elementWithGroup.content_group
          ? {
              id:
                contentGroups.find((g) => g.id === elementWithGroup.content_group?.id)?.codename ??
    ...
    switch (baseElement.type) {
      ...
      case "custom": {
        const result: ContentTypeElements.ICustomElement = {
          ...baseElement,
          allowed_elements: baseElement.allowed_elements?.map((ref) => ({
            id: otherElements.find((el) => el.id === ref.id)?.codename ?? "non-existing-element",
          })),
        };
        ...
        const result: ContentTypeElements.IGuidelinesElement = {
          ...
          guidelines: replaceRichTextReferences({
            ...
            replaceItemId: (itemId, asInternalId, asExternalId) => {
              ...
              return item ? asInternalId(item.codename) : asExternalId("non-existing-item");
            },
            replaceAssetId: (assetId, asInternalId, asExternalId) => {
              const asset = data.assets.find((a) => a.id === assetId);
              return asset ? asInternalId(asset.codename) : asExternalId("non-existing-asset");
            },
            ...
            .replaceAll(
              assetExternalIdRegex,
              () => `${assetExternalIdAttributeName}="non-existing-asset"`,
            )
            .replaceAll(
              itemExternalIdRegex,
              () => `${itemExternalIdAttributeName}="non-existing-item"`,
            )
            .replaceAll(
              itemLinkExternalIdRegex,
              () => `${itemLinkExternalIdAttributeName}="non-existing-item"`,
            ),
        };
      ...
      case "multiple_choice": {
        const result: ContentTypeElements.IMultipleChoiceElement = {
          ...baseElement,
          options: baseElement.options.map((o) => ({
            ...o,
            id: o.codename ?? "non-existing-option",
            external_id: "-",
          })),
          default: baseElement.default
            ? {
                global: {
                  value: baseElement.default.global.value.map((ref) => ({
                    id:
                      baseElement.options.find((o) => o.id === ref.id)?.codename ??
                      "non-existing-option",
                  })),
                },
              }
      ...
      case "snippet": {
        const result: ContentTypeElements.ISnippetElement = {
          ...
          snippet: {
            id:
              data.snippets.find((s) => s.id === baseElement.snippet.id)?.codename ??
      ...
      case "subpages": {
        type SubPagesElement = ContentTypeElements.ISubpagesElement &
      ...
      case "taxonomy": {
        const result: ContentTypeElements.ITaxonomyElement = {
          ...
          taxonomy_group: {
            id:
              data.taxonomies.find((g) => g.id === baseElement.taxonomy_group.id)?.codename ??
              "non-existing-group",
          },
          default: baseElement.default
            ? {
                global: {
                  value: baseElement.default.global.value.map((ref) => ({
                    id:
                      getAllTerms(
                        data.taxonomies.find((g) => g.id === baseElement.taxonomy_group.id),
                      ).find((t) => t.id === ref.id)?.codename ?? "non-existing-term",
                  })),
                },
              }
            : undefined,
            ...
            element: {
              id: baseElement.depends_on.snippet
                ? (data.snippets
                    ...
                    ?.elements.find((el) => el.id === baseElement.depends_on.element.id)
                    ?.codename ?? "non-existing-element")
                : (otherElements.find((el) => el.id === baseElement.depends_on.element.id)
                    ?.codename ?? "non-existing-element"),
            },
            snippet: baseElement.depends_on.snippet
              ? {
                  id:
                    data.snippets.find((s) => s.id === baseElement.depends_on.snippet?.id)
                      ?.codename ?? "non-existing-snippet",
                }
              : undefined,
          },
        };
...
const createPrepareItemReferences: PrepareReferencesCreator<
...
> = (data) => (item) => ({
  ...
  collection: {
    id:
      data.collections.find((c) => c.id === item.collection.id)?.codename ??
      "non-existing-collection",
  },
  spaces: item.spaces.map((s) => ({
    id: data.spaces.find((sp) => sp.id === s.id)?.codename ?? "non-existing-space",
  })),
});

const createPrepareVariantReferences: PrepareReferencesCreator<
...
> = (data) => (variant) => ({
  ...
  language: {
    id:
      data.languages.find((l) => l.id === variant.language.id)?.codename ?? "non-existing-language",
  },
  workflow: {
    workflow_identifier: {
      id:
        data.workflows.find((wf) => wf.id === variant.workflow.workflow_identifier.id)?.codename ??
  ...
  schedule: {
    // TODO: use variant.schedule once we schedule variants when importing
    publish_time: null,
    unpublish_time: null,
...
const createPrepareVariantElementReferences: PrepareReferencesCreator<
  ...
  switch (typeElement.type) {
    ...
    case "modular_content": {
      ...
      const result: LanguageVariantElements.ILinkedItemsInVariantElement = {
        ...
        value:
          typedElement.value?.map((ref) => ({
            id: data.items.find((a) => a.id === ref.id)?.codename ?? "non-existing-item",
          })) ?? [],
      };
    ...
    case "multiple_choice": {
      ...
      const result: LanguageVariantElements.IMultipleChoiceInVariantElement = {
        ...
        value:
          typedElement.value?.map((ref) => ({
            id:
              typedTypeElement.options.find((o) => o.id === ref.id)?.codename ??
              "non-existing-option",
          })) ?? [],
      };
        ...
        value:
          typedElement.value?.map((ref) => ({
            id:
              ...
              ).find((t) => t.id === ref.id)?.codename ?? "non-existing-asset",
          })) ?? [],
      };
    ...
    default:
      throw new Error(
        `Found element of an unknown type "${typeElement.type}". This should not happen.`,
      );
  }
};
      ...
      content_item: webhook.delivery_triggers.content_item
        ? {
            ...
            filters: webhook.delivery_triggers.content_item.filters
              ? {
                  collections: webhook.delivery_triggers.content_item.filters.collections?.map(
                    (ref) => ({
                      id:
                        data.collections.find((c) => c.id === ref.id)?.name ??
                        "non-existing collection",
                    }),
                  ),
                  content_types: webhook.delivery_triggers.content_item.filters.content_types?.map(
                    (ref) => ({
                      id: data.types.find((t) => t.id === ref.id)?.name ?? "non-existing type",
                    }),
                  ),
                  languages: webhook.delivery_triggers.content_item.filters.languages?.map(
                    (ref) => ({
                      id:
                        data.languages.find((l) => l.id === ref.id)?.name ??
                        "non-existing language",
                    }),
                  ),
                }
        ...
        ? {
            ...
            filters: webhook.delivery_triggers.content_type.filters
              ? {
                  ...
                  content_types: webhook.delivery_triggers.content_type.filters.content_types?.map(
                    (ref) => ({
                      id: data.types.find((t) => t.id === ref.id)?.name ?? "non-existing type",
                    }),
                  ),
        ...
        ? {
            ...
            filters: webhook.delivery_triggers.taxonomy.filters
              ? {
                  ...
                  taxonomies: webhook.delivery_triggers.taxonomy.filters.taxonomies?.map((ref) => ({
                    id:
                      data.taxonomies.find((t) => t.id === ref.id)?.name ?? "non-existing taxonomy",
                  })),
                }
              : undefined,
          }
        ...
        ? {
            ...
            filters: webhook.delivery_triggers.language.filters
              ? {
                  ...
                  languages: webhook.delivery_triggers.language.filters.languages?.map((ref) => ({
                    id:
                      data.languages.find((l) => l.id === ref.id)?.name ?? "non-existing language",
                  })),
                }
              : undefined,
          }
