...
import {
  expectDifferentAllEnvData,
  expectSameAllEnvData,
  prepareReferences,
} from "../backupRestore/utils/compare.ts";
import { type AllEnvData, loadAllEnvData } from "../backupRestore/utils/envData.ts";
...
const prepareLanguages = (allData: AllEnvData): AllEnvData => ({
  ...
  languages: allData.languages
    ...
    .toSorted((a, b) => a.codename.localeCompare(b.codename)),
});
...
describe.concurrent("Sync model of two environments with credentials", () => {
  ...
  it.concurrent(
    ...
    withTestEnvironment(SYNC_TARGET_TEST_ENVIRONMENT_ID, async (environmentId) => {
      const command = `sync run -s=${SYNC_SOURCE_TEST_ENVIRONMENT_ID} --sk=${API_KEY} -t=${environmentId} --tk=${API_KEY} --entities contentTypes contentTypeSnippets taxonomies --verbose --skipConfirmation`;
...
describe.concurrent("Partial sync environment from folder", () => {
  ...
  it.sequential("generate  partial sync model test", async () => {
    const command = `sync snapshot -e ${SYNC_SOURCE_TEST_ENVIRONMENT_ID} --entities contentTypes contentTypeSnippets taxonomies collections -k ${API_KEY} -f ${folderPath}`;
    await runCommand(command);
    ...
    const filesShouldNotExist = Object.values(fileNames).filter(
      (f) => !filesShouldExist.includes(f),
    );
  ...
  it.sequential(
    ...
    withTestEnvironment(SYNC_TARGET_TEST_ENVIRONMENT_ID, async (environmentId) => {
      const command = `sync run -t=${environmentId} --tk=${API_KEY} -f=${folderPath} --entities=contentTypes contentTypeSnippets taxonomies --verbose --skipConfirmation`;
