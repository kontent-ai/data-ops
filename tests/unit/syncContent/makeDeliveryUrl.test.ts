...
describe("makeDeliveryUrl", () => {
  it("should return correct parameters for last parameter", () => {
    const last = 10;
    const params: DeliveryParamsOptions = {
      language: "default",
      limit: last,
    };
...
describe("getDeliveryUrlParams", () => {
  ...
  it("should return correct deliver url parameters for byTypeCodenames cli param", () => {
    const byTypeCodenames = ["type1", "type2"];
    ...
    expect(parameters).toStrictEqual([
      ...
      new Filters.InFilter("system.type", byTypeCodenames),
    ]);
  });
